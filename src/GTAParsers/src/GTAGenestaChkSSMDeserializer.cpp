...
AINGTAGenestaChkSSMDeserializer::AINGTAGenestaChkSSMDeserializer(const QString& iStmt, const QString& isEquipment, const QString& isAircraft,const QString &iPrecision):AINGTAGenestaParserItf(iStmt)
{
...
bool AINGTAGenestaChkSSMDeserializer::getCommands(QList<AINGTACommandBase*>& oLstCmds)
{
    bool rC = false;
    ...
    if (!_statement.isEmpty())
    {
        ...
        sCheckValParamStatement.remove(GENESTA_CMD_CHECK_SSM);
            ...
            if (!sParameterValInfo.isEmpty())
            {
                ...
                foreach(QString sComparator, sComparators)
                {
                    if (sParameterValInfo.contains(QString(" %1 ").arg(sComparator)))
                    {
                        ...
                        QStringList paramValInfoLst = sParameterValInfo.split(QString(" %1 ").arg(sComparator));
                        if(paramValInfoLst.size() == 2)
                        {
                            ...
                            QStringList lstofValInfo = QStringList()<<sParamValInfo;
 ...
 //                           {
                                ...
                                for (int i=0;i<lstofParams.size();i++)
                                {
                                    for (int j=0;j<lstofValInfo.size();j++)
                                    {
                                        ...
                                        //  pChkValue->insertParamenter(sParamName,sComparator,"",sFunctionalStatus,_Precision.toDouble(),ACT_PRECISION_UNIT_VALUE,false,true);

                                        if(lstofParams.size() == 2 && _statement.contains("both channels"))
                                        {
                                            if(i ==0)
                                                pChkValue->insertParamenter(sParamName,sComparator,sparamVal,sFunctionalStatus,_Precision.toDouble(),ACT_PRECISION_UNIT_VALUE,false,true);
                                            else
                                            {
                                                pChkValue2->insertParamenter(sParamName,sComparator,sparamVal,sFunctionalStatus,_Precision.toDouble(),ACT_PRECISION_UNIT_VALUE,false,true);
                                            }
                                        }
                                        else
                                            pChkValue->insertParamenter(sParamName,sComparator,sparamVal,sFunctionalStatus,_Precision.toDouble(),ACT_PRECISION_UNIT_VALUE,false,true);
...
bool  AINGTAGenestaChkSSMDeserializer::getLstOfCommandsForChannelInCTRL(QList<AINGTACommandBase*>& oLstCmds,QStringList& iLstofParam,QStringList& iLstofVal,QString sComparator)
{
    bool rC;
    ...
    //if (!chsForEn1.isEmpty() && !chsForEn2.isEmpty())
    {
        ...
        if(iLstofParam.size()==4  )
        {
            ...
            pIF11->setValue("1");
            pIF11->setCondition(sComparator);
            ...
            pIF21->setValue("1");
            pIF21->setCondition(sComparator);
            ...
            if(iLstofVal.size()==4)
            {
                ...
                pCh111->insertParamenter(p_chA_en1,sComparator,v_chA_en1,sFunctionalStatus,_Precision.toDouble(),ACT_PRECISION_UNIT_VALUE,false,true);
                pCh111->insertBinaryOperator("OR");
                pCh111->insertParamenter(p_chA_en2,sComparator,v_chA_en2,sFunctionalStatus,_Precision.toDouble(),ACT_PRECISION_UNIT_VALUE,false,true);
                pIF11->insertChildren(pCh111,0);

                pCh112->insertParamenter(p_chA_en1,sComparator,v_chA_en1,sFunctionalStatus,_Precision.toDouble(),ACT_PRECISION_UNIT_VALUE,false,true);
                pCh112->insertBinaryOperator("OR");
                pCh112->insertParamenter(p_chB_en2,sComparator,v_chB_en2,sFunctionalStatus,_Precision.toDouble(),ACT_PRECISION_UNIT_VALUE,false,true);
                ...
                pCh211->insertParamenter(p_chB_en1,sComparator,v_chB_en1,sFunctionalStatus,_Precision.toDouble(),ACT_PRECISION_UNIT_VALUE,false,true);
                pCh211->insertParamenter(p_chA_en2,sComparator,v_chA_en2,sFunctionalStatus,_Precision.toDouble(),ACT_PRECISION_UNIT_VALUE,false,true);
                pCh211->insertBinaryOperator("OR");
                pIF21->insertChildren(pCh211,0);

                pIF21->insertChildren(pElse21,1);

                pCh212->insertParamenter(p_chB_en1,sComparator,v_chB_en1,sFunctionalStatus,_Precision.toDouble(),ACT_PRECISION_UNIT_VALUE,false,true);
                pCh212->insertParamenter(p_chB_en2,sComparator,v_chB_en2,sFunctionalStatus,_Precision.toDouble(),ACT_PRECISION_UNIT_VALUE,false,true);
            ...
            else if(iLstofVal.size()==2)
            {
                ...
                pCh111->insertParamenter(p_chA_en1,sComparator,v_chA_en1,sFunctionalStatus,_Precision.toDouble(),ACT_PRECISION_UNIT_VALUE,false,true);
                pCh111->insertBinaryOperator("OR");
                pCh111->insertParamenter(p_chA_en2,sComparator,v_chA_en1,sFunctionalStatus,_Precision.toDouble(),ACT_PRECISION_UNIT_VALUE,false,true);
                pIF11->insertChildren(pCh111,0);

                pCh112->insertParamenter(p_chA_en1,sComparator,v_chA_en1,sFunctionalStatus,_Precision.toDouble(),ACT_PRECISION_UNIT_VALUE,false,true);
                pCh112->insertBinaryOperator("OR");
                pCh112->insertParamenter(p_chB_en2,sComparator,v_chB_en1,sFunctionalStatus,_Precision.toDouble(),ACT_PRECISION_UNIT_VALUE,false,true);
                ...
                pElse2->insertChildren(pIF21,0);
                pCh211->insertParamenter(p_chB_en1,sComparator,v_chB_en1,sFunctionalStatus,_Precision.toDouble(),ACT_PRECISION_UNIT_VALUE,false,true);
                pCh211->insertParamenter(p_chA_en2,sComparator,v_chA_en1,sFunctionalStatus,_Precision.toDouble(),ACT_PRECISION_UNIT_VALUE,false,true);
                pCh211->insertBinaryOperator("OR");
                pIF21->insertChildren(pCh211,0);
                ...
                pCh212->insertParamenter(p_chB_en1,sComparator,v_chB_en1,sFunctionalStatus,_Precision.toDouble(),ACT_PRECISION_UNIT_VALUE,false,true);
                pCh212->insertParamenter(p_chB_en2,sComparator,v_chB_en1,sFunctionalStatus,_Precision.toDouble(),ACT_PRECISION_UNIT_VALUE,false,true);
                pCh212->insertBinaryOperator("OR");
                pElse21->insertChildren(pCh212,0);
            ...
            else if(iLstofVal.size()==1)
            {
                ...
                pCh112->insertParamenter(p_chA_en1,sComparator,v_chA_en1,sFunctionalStatus,_Precision.toDouble(),QString(""),ACT_PRECISION_UNIT_VALUE,true);
                pCh112->insertBinaryOperator("OR");
                pCh112->insertParamenter(p_chB_en2,sComparator,v_chA_en1,sFunctionalStatus,_Precision.toDouble(),QString(""),ACT_PRECISION_UNIT_VALUE,true);
                ...
                pElse2->insertChildren(pIF21,0);
                pCh211->insertParamenter(p_chB_en1,sComparator,v_chA_en1,sFunctionalStatus,_Precision.toDouble(),ACT_PRECISION_UNIT_VALUE,false,true);
                pCh211->insertParamenter(p_chA_en2,sComparator,v_chA_en1,sFunctionalStatus,_Precision.toDouble(),QString(""),ACT_PRECISION_UNIT_VALUE,true);
                pCh211->insertBinaryOperator("OR");
                pIF21->insertChildren(pCh211,0);

                pIF21->insertChildren(pElse21,1);

                pCh212->insertParamenter(p_chB_en1,sComparator,v_chA_en1,sFunctionalStatus,_Precision.toDouble(),ACT_PRECISION_UNIT_VALUE,false,true);
                pCh212->insertParamenter(p_chB_en2,sComparator,v_chA_en1,sFunctionalStatus,_Precision.toDouble(),ACT_PRECISION_UNIT_VALUE,false,true);
                pCh212->insertBinaryOperator("OR");
                pElse21->insertChildren(pCh212,0);
        ...
        else if(iLstofVal.size()==4  )
        {
            ...
            if(iLstofParam.size()==2)
            {
                ...
                pCh111->insertParamenter(p_chA_en1,sComparator,v_chA_en1,sFunctionalStatus,_Precision.toDouble(),ACT_PRECISION_UNIT_VALUE,false,true);
                pCh111->insertBinaryOperator("OR");
                pCh111->insertParamenter(p_chA_en1,sComparator,v_chA_en2,sFunctionalStatus,_Precision.toDouble(),ACT_PRECISION_UNIT_VALUE,false,true);
                pIF11->insertChildren(pCh111,0);

                pCh112->insertParamenter(p_chA_en1,sComparator,v_chA_en1,sFunctionalStatus,_Precision.toDouble(),ACT_PRECISION_UNIT_VALUE,false,true);
                pCh112->insertBinaryOperator("OR");
                pCh112->insertParamenter(p_chB_en1,sComparator,v_chB_en2,sFunctionalStatus,_Precision.toDouble(),ACT_PRECISION_UNIT_VALUE,false,true);
                ...
                pElse2->insertChildren(pIF21,0);
                pCh211->insertParamenter(p_chB_en1,sComparator,v_chB_en1,sFunctionalStatus,_Precision.toDouble(),ACT_PRECISION_UNIT_VALUE,false,true);
                pCh211->insertParamenter(p_chA_en1,sComparator,v_chA_en2,sFunctionalStatus,_Precision.toDouble(),ACT_PRECISION_UNIT_VALUE,false,true);
                pCh211->insertBinaryOperator("OR");
                pIF21->insertChildren(pCh211,0);
                ...
                pCh212->insertParamenter(p_chB_en1,sComparator,v_chB_en1,sFunctionalStatus,_Precision.toDouble(),ACT_PRECISION_UNIT_VALUE,false,true);
                pCh212->insertParamenter(p_chB_en1,sComparator,v_chB_en2,sFunctionalStatus,_Precision.toDouble(),ACT_PRECISION_UNIT_VALUE,false,true);
                pCh212->insertBinaryOperator("OR");
                pElse21->insertChildren(pCh212,0);
            ...
            else if(iLstofParam.size()==1)
            {
                ...
                pElse2->insertChildren(pIF21,0);
                pCh211->insertParamenter(p_chA_en1,sComparator,v_chA_en1,sFunctionalStatus,_Precision.toDouble(),ACT_PRECISION_UNIT_VALUE,false,true);
                pCh211->insertParamenter(p_chA_en1,sComparator,v_chA_en1,sFunctionalStatus,_Precision.toDouble(),ACT_PRECISION_UNIT_VALUE,false,true);
                pCh211->insertBinaryOperator("OR");
                pIF21->insertChildren(pCh211,0);
                ...
                pCh212->insertParamenter(p_chA_en1,sComparator,v_chA_en1,sFunctionalStatus,_Precision.toDouble(),ACT_PRECISION_UNIT_VALUE,false,true);
                pCh212->insertParamenter(p_chA_en1,sComparator,v_chA_en1,sFunctionalStatus,_Precision.toDouble(),ACT_PRECISION_UNIT_VALUE,false,true);
                pCh212->insertBinaryOperator("OR");
                pElse21->insertChildren(pCh212,0);
        ...
        if (iLstofParam.size()==2 && iLstofVal.size()==1)
        {
            ...
            pCh111->insertParamenter(p_chA,sComparator,v,sFunctionalStatus,_Precision.toDouble(),ACT_PRECISION_UNIT_VALUE,false,true);
            pCh211->insertParamenter(p_chB,sComparator,v,sFunctionalStatus,_Precision.toDouble(),ACT_PRECISION_UNIT_VALUE,false,true);
        ...
        else  if (iLstofVal.size()==2 && iLstofParam.size()==1)
        {
            ...
            pCh111->insertParamenter(p,sComparator,v_chA,sFunctionalStatus,_Precision.toDouble(),ACT_PRECISION_UNIT_VALUE,false,true);
            pCh211->insertParamenter(p,sComparator,v_chB,sFunctionalStatus,_Precision.toDouble(),ACT_PRECISION_UNIT_VALUE,false,true);
...
QStringList AINGTAGenestaChkSSMDeserializer::getChannelSelectedForSignals(const QStringList& iSignalListLHS,const QStringList& iSignalListRHS)const
{
    QStringList channelSelectedLst;
...
void AINGTAGenestaChkSSMDeserializer::getPreprocessedStatements(const QString& iStatement, QStringList& oList)
{
