...
GTACmdSerializerInterface::GTACmdSerializerInterface()
{
}
bool GTACmdSerializerInterface::setActionAttributes(GTAActionBase* pAct,  QDomElement& actionElementNode)
{
    bool rc = false;
    if (pAct!=NULL && actionElementNode.isNull()==false)
    {
        ...
        QString uuid = pAct->getObjId();
        actionElementNode.setAttribute(XNODE_UUID,uuid);
...
bool GTACmdSerializerInterface::setActionAttributes(GTACheckBase* pChk,  QDomElement& actionElementNode)
{
    bool rc = false;
...
bool GTACmdSerializerInterface::getActionAttributes(const QDomElement& iElement,GTAActionBase* pAct)
{
    ...
    if(!iElement.isNull() && iElement.nodeName() == XNODE_ACTION && pAct!=NULL)
    {
        ...
        if (uuid == "")
            ...
            pAct->setObjId(uuid,false);

    }

    return rc;
}
bool GTACmdSerializerInterface::getActionAttributes(const QDomElement& iElement,GTACheckBase* pChk)
{
