...
GTAActionIF::GTAActionIF()
{
    ...
    setComplement(COM_CONDITION_IF);
}
GTAActionIF::GTAActionIF(const GTAActionIF& rhs):GTAActionBase(rhs)
{
    _ChkAudioAlarm = false;
    _IsFSOnly = false;
    _IsValueOnly = false;
    ...
    this->setParent(rhs.getParent());//This should be set based on paste
...
GTACommandBase* GTAActionIF::getClone() const
{
    GTACommandBase* pBase = new GTAActionIF(*this);
    return pBase;
}
GTAActionIF::~GTAActionIF()
{
...
QString GTAActionIF::getGlobalResultStatus()
{
    QString data = "NA";
    QString data2 = getExecutionResult();
...
void GTAActionIF::setParameter(const QString & iParam)
{
    _Param =iParam;
}

void GTAActionIF::setValue(const QString & iParam)
{
    _ParamValue = iParam;
}
QString GTAActionIF::getValue()const
{
    return _ParamValue;
}
void GTAActionIF::setCondition(const QString & iOperator)
{
    _Operator = iOperator;
}

void GTAActionIF::setChkAudioAlarm(const bool iChkAudioAlarm)
{
    _ChkAudioAlarm = iChkAudioAlarm;
}
bool GTAActionIF::getChkAudioAlarm()const
{
    return _ChkAudioAlarm;
}

QString GTAActionIF::getParameter() const
{
    return _Param;
}

QString GTAActionIF::getCondition() const
{
    return _Operator;
}

QString GTAActionIF::getStatement() const
{
    QString oAction;
    ...
    if(getIsFsOnly())
    {
        oAction = QString("%1 (Function Status of %2 %4 %3)").arg("IF",paramName,getFunctionalStatus(),getCondition());
    }
    else if(getIsSDIOnly())
    {
        oAction = QString("%1 (SDI value of %2 %4 %3)").arg("IF",paramName,getSDI(),getCondition());
    }
    else if(getIsParityOnly())
    {
        oAction = QString("%1 (Parity of %2 %4 %3)").arg("IF",paramName,getParity(),getCondition());
    }
    else if(getIsRefreshRateOnly())
    {
        oAction = QString("%1 (Refresh rate is set for %2)").arg("IF",paramName);
    }
    else
        oAction = QString("%1 %2 %3 %4").arg("IF",paramName,getCondition(),getValue());
    return oAction;
}

QString GTAActionIF::getLTRAStatement() const
{
    QString oAction;
    QString paramName = getParameter();
    if(getIsFsOnly())
    {
        oAction = QString("#c#%1#c# (Function Status of #b#%2#b# %4 #b#%3#b#)").arg("IF",paramName,getFunctionalStatus(),getCondition());
    }
    else if(getIsSDIOnly())
    {
        oAction = QString("#c#%1#c# (SDI value of #b#%2#b# %4 #b#%3#b#)").arg("IF",paramName,getSDI(),getCondition());
    }
    else if(getIsParityOnly())
    {
        oAction = QString("#c#%1#c# (Parity of #b#%2#b# %4 #b#%3#b#)").arg("IF",paramName,getParity(),getCondition());
    }
    else if(getIsRefreshRateOnly())
    {
        oAction = QString("#c#%1#c# (Refresh rate is set for #b#%2#b#)").arg("IF",paramName);
    }
    else
        oAction = QString("#c#%1#c# #b#%2#b# %3 #b#%4#b#").arg("IF",paramName,getCondition(),getValue());
    return oAction;
}

QString GTAActionIF::getSCXMLStateName()const
{
...
QList<GTACommandBase*>& GTAActionIF::getChildren(void) const
{
    return *_lstChildren;
}
bool GTAActionIF::insertChildren(GTACommandBase* pBase,const int& idPos)
{
    bool rc = false;
...
void  GTAActionIF::setParent(GTACommandBase* iParent)
{
    _parent=iParent;
}
GTACommandBase* GTAActionIF::getParent(void) const
{
    return _parent;
}
//bool GTAActionIF::hasElse()
//{
//    bool rc = false;
//    int currSize=_lstChildren->size();
...
//    return rc;
//}
bool GTAActionIF:: hasChildren()
{
    return !(_lstChildren->isEmpty());
}
bool GTAActionIF::canHaveChildren() const
{
    return true;
}
...
/* Old implementation*/
QStringList GTAActionIF::resolveEngineParam()const
{
...
QStringList GTAActionIF::resolveEngineParam(const QString &iParam)const
{
    QStringList lstOfVars;
...
QStringList GTAActionIF::getVariableList() const
{
    QStringList lstOfVars;
...
void GTAActionIF:: ReplaceTag(const QMap<QString,QString>& iTagValueMap) 
{
...
bool GTAActionIF::hasMutuallExclusiveStatement(int* opPos)const
{
    bool rc = false;
    int currSize=_lstChildren->size();
    int elseCounter=0;
    for(int i=0;i<currSize;i++)
    {
        ...
        if (pCmd)
        {
            ...
            elseCounter = elseCounter+pCmd->getAllChildrenCount();
        }
    }
    return rc;
}
bool GTAActionIF::canHaveMutuallExclusiveStatement()const
{
    return true;
}
void GTAActionIF::stringReplace(const QRegExp& iStrToFind,const QString& iStringToReplace)
{
    _Param.replace(iStrToFind,iStringToReplace);
    _ParamValue.replace(iStrToFind,iStringToReplace);
}
bool GTAActionIF::equipmentReplace(const QRegExp& iStrToFind, const QString& iStringToReplace)
{
    QString temp = _Param;
    bool rc = false;
...
bool GTAActionIF::searchString(const QRegExp& iRegExp,bool ibTextSearch)const
{
    bool rc = false;
...
bool GTAActionIF::expandReferences()
{
...
bool GTAActionIF::setDumpList(const QStringList& iDumpList)
{
    _dumpList.clear();
    _dumpList.append(iDumpList);
    return true;
}
QStringList GTAActionIF::getDumpList()const
{
    return _dumpList;
    
}

void GTAActionIF::setHeardAfter(const QString & iHeardAfter)
{
    _HeardAfter = iHeardAfter;
}
QString GTAActionIF::getHeardAfter() const
{
    return _HeardAfter;
}


bool GTAActionIF::getSCXMLConditionStatement(const QStringList &iIcdParamList, QString &oCondtionStatement, const QString &iEngine, bool isGenericSCXML, const QStringList iLocalVarList) const
{
    ...
    if(isGenericSCXML)
    {
        ...
        lhs.toDouble(&isLhsDouble);
        if(!isLhsDouble && lhs.contains("."))
            lhs = lhs.split(".").last();

        bool isRhsDouble = false;
        rhs.toDouble(&isRhsDouble);
        if(!isRhsDouble && rhs.contains("."))
            rhs = rhs.split(".").last();
    }
    if(getChkAudioAlarm())
    {
...
QString GTAActionIF::getSCXMLCondition(const QString &iLhs, const QString &iRhs, const QStringList &iIcdParamList, bool isGenericSCXML, const QStringList iLocalVarList) const
{
...
//    if (iRhs.contains(regExp))
//    {
//        QString iRhstemp = iRhs.trimmed();
        rhsVal = resolveInternalParams(iRhs,iLocalVarList);
        //obtaining list of regx in rhs
//        QString iRhstemp = iRhs;
//        int pos = regExp.indexIn(iRhs);
//        QStringList listRHS = regExp.capturedTexts();
//        QString idinRHS = listRHS.first();
//        foreach (QString var, iLocalVarList)
//        {
//            QString vartemp = var;
//            QString colName = iRhstemp.remove(regExp).remove("_uuid").split(".").last();
//            QString colNameinLst = vartemp.remove(regExp).split(".").last();
//            if (var.contains(idinRHS) && (colName == colNameinLst))
//            {
...
//        }
//    }
//    if (iLhs.contains(regExp))
//    {
//        QString iLhstemp = iLhs;
        lshVal = resolveInternalParams(iLhs,iLocalVarList);
        //obtaining list of regx in lhs
//        QString iLhstemp = iLhs;
//        int pos = regExp.indexIn(iLhs);
//        QStringList listLHS = regExp.capturedTexts();
//        QString idinLHS = listLHS.first();
//        foreach (QString var, iLocalVarList)
//        {
//            QString vartemp = var;
//            QString colName = iLhstemp.remove(regExp).remove("_uuid").split(".").last();
//            QString colNameinLst = vartemp.remove(regExp).split(".").last();
//            if (var.contains(idinLHS) && (colName == colNameinLst))
//            {
//                lshVal = var.remove(idinLHS);
//                break;
//            }
//        }
//    }
    ...
    if(isLhsParam)
    {
        ...
        if(!isGenericSCXML)
        {
            ...
            {	
                if(fsVal != ACT_FSSSM_NOT_LOST && !fsVal.isEmpty())
                    lhsStatusCond = QString("%1.Status == '%2'").arg(lhs,fsVal);
                else
...
QString GTAActionIF::precisionCondition(const QString &iLhsVal, const QString &iRhsVal,const QString &op,const QString &iPrecision,const QString &precisionUnit,double dPrec, bool isGenericSCXML)const
{
    QString ValCond;
...
void GTAActionIF::replaceUntagged()
{
    int currChildrenSize = _lstChildren->size();
    for (int i=currChildrenSize-2;i>=0;i--)
    {
...
bool GTAActionIF::createEndCommand(GTACommandBase* & opCmd)const
{
    opCmd = new GTAActionIfEnd;
    return true;

}
bool GTAActionIF::hasChannelInControl()const
{
...
void GTAActionIF::setIsFsOnly(const bool &iVal)
{
    _IsFSOnly = iVal;
}

bool GTAActionIF::getIsFsOnly()const
{
    return _IsFSOnly;
}

void GTAActionIF::setIsValueOnly(const bool &iVal)
{
    _IsValueOnly = iVal;
}

bool GTAActionIF::getIsValueOnly()const
{
    return _IsValueOnly;
}

QString GTAActionIF::getFunctionalStatus()const
{
    return _FunctionalStatus;
}

void GTAActionIF::setFunctionalStatus(const QString &iFuncStatus)
{
