...
GTAActionFunctionReturnSerializer::GTAActionFunctionReturnSerializer() : GTACmdSerializerInterface()
{
    _pActionCmd = NULL;
}

void GTAActionFunctionReturnSerializer::setCommand(const GTACommandBase* ipCmd)
{
    _pActionCmd = dynamic_cast<GTAActionFunctionReturn*>((GTACommandBase*) ipCmd);
}
bool GTAActionFunctionReturnSerializer::serialize(QDomDocument &iDomDoc, QDomElement &oElement)//(QDomDocument &iDomDoc,QDomElement &oElement)
{
    bool rc = false;
    if(_pActionCmd != NULL)
    {
        ...
        QString sParameter = _pActionCmd->getReturnItem();
...
bool GTAActionFunctionReturnSerializer::deserialize(const QDomElement &iElement, GTACommandBase *&opCommand)//(const QDomElement & ioElement)
{
    bool rc = false;
    if(!iElement.isNull())
    {
        ...
        GTAActionFunctionReturn* pReturnFuncCmd = new GTAActionFunctionReturn();
		
        /*pReturnFuncCmd->setActionOnFail(sOnFail);
        pReturnFuncCmd->setTimeOut(sTimeOut,sTimeOutUnit);
        pReturnFuncCmd->setPrecision(sPrecision,sPrecisionUnit);
        pReturnFuncCmd->setIgnoreInSCXML(ignoreInSCXML);*/

        getActionAttributes(iElement,pReturnFuncCmd);
        ...
        for(int i = 0; i < childList.count(); i++)
        {
            ...
            if(!childElem.isNull())
            {
                ...
                if(nodeName == XNODE_PARAMETER)
                {
                        ...
                        pReturnFuncCmd->setReturnItem(paramVals);
                    
                }
            }
        }

        opCommand = pReturnFuncCmd;
...
REGISTER_SERIALIZER(QString("%1").arg(ACT_FUNC_RETURN),GTAActionFunctionReturnSerializer)
